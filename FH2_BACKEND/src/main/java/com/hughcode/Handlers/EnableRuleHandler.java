...
public class EnableRuleHandler implements HttpHandler {

    @Override
    public void handle(HttpExchange exchange) throws IOException {
        try {
            int ruleId = getRuleId(exchange);
            RulesTableDAO.enableRule(ruleId);
            SendResponse.sendResponse(exchange, 200, "Rule enabled");
        } catch (IllegalArgumentException e) {
            SendResponse.sendResponse(exchange, 400, "{\"error\":\"A valid rule_id is required\"}");
        } catch (Exception e) {
            System.out.println("Error enabling rule: " + e);
